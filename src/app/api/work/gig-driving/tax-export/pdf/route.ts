...
import { buildTaxSummaryPdf } from "@/lib/work/gig-tax-export-files";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const pdf = await buildTaxSummaryPdf(data);
    return new NextResponse(new Uint8Array(pdf), {
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${data.taxYear}-Gig-Tax-Summary.pdf"`,
      },
    });
  } catch (err) {
